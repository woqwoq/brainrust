...
impl<I: Read, O: Write> Interpreter<I, O> {
    ...
    pub fn has_instruction(&self) -> bool {
        self.program.has_instruction(self.program_counter)
    }
...
mod interpreter_tests {
    ...
    /// Single place in tests that constructs an `Interpreter` — when the
    /// constructor changes, fix it here and nowhere else.
    fn test_interpreter_with<I: Read, O: Write>(
        code: &str,
        input: I,
        output: O,
    ) -> Interpreter<I, O> {
    ...
    fn test_interpreter(code: &str) -> Interpreter<io::Empty, Vec<u8>> {
        test_interpreter_with(code, io::empty(), Vec::new())
    }
    ...
    fn parametrized_constructs_properly(code: &str) {
        ...
        assert_eq!(vec![0u8; interpreter.memory.len()], interpreter.memory);
    }
    ...
    fn returns_error_on_tape_overflow() {
        let mut interpreter = test_interpreter("");
        interpreter.memory_pointer = interpreter.memory.len();
    ...
    fn returns_error_on_memory_access_out_of_range() {
        ...
        assert!(matches!(
            interpreter.fetch_memory(interpreter.memory.len()),
    ...
    fn returns_error_on_memory_access_out_of_range_mut() {
        ...
        assert!(matches!(
            interpreter.fetch_memory_mut(interpreter.memory.len()),
            Err(RuntimeError::MemoryAccessOutOfBounds { .. })
        ));
    }
    ...
    fn handle_input_stores_single_bytes() {
        let mut interpreter = test_interpreter_with("", Cursor::new(b"ab".to_vec()), Vec::new());
